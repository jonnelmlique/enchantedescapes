<?php
	...
	$q_student = $conn->query("SELECT * FROM `student` WHERE `student_no` = '$student'") or mysqli_error($mysql);
	
	$f_student = $q_student->fetch_array();
	$student_name = $f_student['firstname']." ".$f_student['lastname'];
	$conn->query("INSERT INTO `time` VALUES('', '$student', '$student_name', '$time', '$date')") or mysqli_error($mysql);
