...
try {
    ...
    $sql = "SELECT ri.roominfoid, ri.roomtype, ri.roomnumber, ri.roomfloor,
        CONCAT(rs.firstname, ' ', rs.lastname) AS full_name,
        rs.mobilenumber, rs.adults, rs.children, rs.checkindate, rs.checkintime, rs.checkoutdate, rs.checkouttime, rs.price, rs.totalafterpromo, rs.status
...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sqlInsertChecked = "INSERT INTO checked (roominfoid, guestname, contactno, roomnumber, roomfloor, roomtype, price, checkindate, checkintime, checkoutdate, checkouttime, adult, children, status, reservationfee) 
                            VALUES (:roominfoid, :full_name, :contactno, :roomnumber, :roomfloor, :roomtype, :price, :checkindate, :checkintime, :checkoutdate, :checkouttime, :adult, :children, 1, :reservationfee)";
        $stmtInsertChecked = $pdo->prepare($sqlInsertChecked);
        $stmtInsertChecked->execute(
            array(
        ...
        // Update the status in the reservationprocess table to 'Check-In'
        $updateQuery = "UPDATE reservationprocess rs
INNER JOIN roominfo ri ON ri.roomnumber = rs.roomnumber
SET rs.status = 'Check-In'
WHERE CONCAT(rs.firstname, ' ', rs.lastname) = ?";
        $stmtUpdate = $pdo->prepare($updateQuery);
        $stmtUpdate->execute([$full_name]);
...
<head>
    ...
    .search-bar input {
        ...
        outline: none;
        margin-right: 40px;
        /* Adjusted margin to avoid overlapping */
    }
    ...
    .table td {
        ...
        /* Added to show ellipsis for overflow */
    }
            ...
            <div class="card bg-white">
                <div class="card-body">
                    ...
                    <div class="table-container">
                        <form method="post" action="booking.php" id="checkInForm">
                            <!-- Add form tags with an action to process_checked.php -->
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td><input type="checkbox" name="row_checkbox[]"
                                                value="<?php echo $row['roominfoid']; ?>"
                                                onclick="handleCheckbox(this)"></td>
                                        ...
                                        <td class="action-buttons">
                                            ...
                                            <!-- Add submit button to trigger form submission -->
                                        </td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
