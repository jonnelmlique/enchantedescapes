...
if (isset($_POST['update'])) {
    ...
    $insertPaymentQuery = "INSERT INTO payments (checkedids, rmtotalamount, ordtotalamount, overallamount, paidamount, balance) VALUES (?, ?, ?, ?, ?, ?)"; // Added balanceamount to INSERT query
    $stmtInsertPayment = $pdo->prepare($insertPaymentQuery);
    $stmtInsertPayment->execute([$checkedIds, $roomTotalPrice, $ordersTotalAmount, $overallAmount, $paidAmount, $balanceAmount]); // Added balance amount to execute
...
<div class="checkin-room">
    ...
    <div class="input-group-modal">
        <div class="input-item">
            ...
            <form id="checkoutForm" action="checkout.php" method="POST" onsubmit="return validateCheckout()"> 
...
document.getElementById('roomTotalPrice').value = roomPrice;
...
<script>
 document.addEventListener("DOMContentLoaded", function() {
    ...
    // Fix the number of days to 1 if check-out date is the same as check-in date
    daysInput.value = diffDays === 0 ? 1 : diffDays;
...
function updateRoomTotalPrice() {
    ...
    const ordersTotalAmount = parseFloat(document.getElementById('totalAmountHidden').value.replace('₱', ''));

    if (!isNaN(days) && !isNaN(roomPrice)) {
        let roomTotalPrice = days * roomPrice;
        if (days === 0) {
            roomTotalPrice = roomPrice; // Set roomTotalPrice to roomPrice for same-day booking
        }
        ...
        calculateOverallAmount(roomTotalPrice, ordersTotalAmount); // Calculate overall amount with updated roomTotalPrice and ordersTotalAmount
        calculateTotal();
    }
}


function calculateOverallAmount(roomTotalPrice, ordersTotalAmount) {
    ...
    // Set the value of hidden input for overall amount directly
...
function calculateTotal() {
    ...
    // Function to update Room Total Price based on Room Price
    function updateRoomTotalPrice() {
        const roomPrice = parseFloat(roomPriceInput.value);
        // Update the Room Total Price input field
        roomTotalPriceInput.value = roomPrice.toFixed(2); // Adjust decimal places as needed
        // Update the hidden Room Total Price input field
        roomTotalPriceHiddenInput.value = roomPrice.toFixed(2); // Store the value without displaying it
    }

    // Add event listener to Room Price input for changes
    roomPriceInput.addEventListener('input', updateRoomTotalPrice);

    // Initial call to update Room Total Price based on initial Room Price
    updateRoomTotalPrice();
    
    function validateCheckout() {
    var checkedId = document.getElementById('checkedId').value;
    var checkOutDate = document.getElementById('checkOutDate').value;
    var checkOutTime = document.getElementById('checkOutTime').value;
    var days = document.getElementById('days').value;
    var roomTotalPrice = document.getElementById('roomTotalPrice').value;
    var paymentMethod = document.getElementById('paymentMethod').value;
    var paidAmount = document.getElementById('paidAmount').value;
    var ovAmountHidden = document.getElementById('ovAmountHidden').value;

    if (checkedId === '' || checkOutDate === '' || checkOutTime === '' || days === '' || roomTotalPrice === '' || paymentMethod === '' || paidAmount === '' || ovAmountHidden === '' || ovAmountHidden == 0) {
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Please fill in all fields and ensure the overall amount is not 0.'
        });
        return false; // Prevent form submission
    }
    ...
    if (parseFloat(paidAmount) < parseFloat(ovAmountHidden)) {
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Paid amount cannot be lower than the overall amount.'
        });
        return false; // Prevent form submission
    }
...
const ovAmountElement = document.getElementById('ovAmountHidden'); // Assuming the hidden input holds the overall amount
...
function calculateBalance() {
    const ovAmount = parseFloat(ovAmountElement.value); // Use .value to get the input value
    const paidAmount = parseFloat(paidAmountElement.value);

    // Check if the inputs are valid numbers
    if (!isNaN(ovAmount) && !isNaN(paidAmount)) {
        const balance = paidAmount - ovAmount;
        balanceAmountElement.value = balance.toFixed(2); // Assuming you want to display the balance with two decimal places
    } else {
        balanceAmountElement.value = ''; // Clear the balance if inputs are not valid
    }
}

// Call the function when inputs change
