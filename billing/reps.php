...
// SQL query to retrieve payment information within the specified date range
$sql = "SELECT * FROM monthly_sales_report WHERE checkoutdate >= ? AND checkoutdate <= ?";
// Prepare the SQL statement
$stmt = $conn->prepare($sql);

if (!$stmt) {
    die("Error preparing statement: " . $conn->error); // Handle the error
}
...
while ($row = $result->fetch_assoc()) {
    $totalSales += $row['room_total_price']; // Add room total price to total sales
}
...
<header>
 ...
 <p>(<?php echo $startDate; ?> - <?php echo $endDate; ?>)</p></h1>
    <address>
        <p>ENCHANTED HOTEL,</p>
    ...
    <table class="inventory">
        ...
        if ($result && $result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                // Add total amount to total sales
                $totalSales += $totalAmount;
            }
        } else {
            // If no results found, display a message
            echo "<tr><td colspan='13'>No payment records found</td></tr>";
        }
        ?>
        </tbody>
        <tfoot>
            <tr>
                <th colspan="12" style="text-align: right;">Total Sales (<?php echo date("F Y"); ?>):</th>
