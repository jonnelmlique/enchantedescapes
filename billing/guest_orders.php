...
try {
    ...
    $sql = "SELECT checkedid, guestname, roominfoid, status FROM checked WHERE status = 1 LIMIT :offset, :records_per_page";
    ...
    if (!empty($_POST['selectedItems'])) {
        ...
        if ($stmt) {
            ...
            if ($stmt->affected_rows >= 1) {
                ...
                foreach ($_POST['selectedItems'] as $stockId) {
                    $orderQty = $_POST['orderQty'][$stockId];
            
                    // Update stocks table to decrease quantity
                    $stmtUpdate = $conn->prepare("UPDATE stock SET quantity_per_item = quantity_per_item - ? WHERE stock_id = ?");
                    $stmtUpdate->bind_param("ii", $orderQty, $stockId);
                    $stmtUpdate->execute();
                    $stmtUpdate->close();
                }
